...
- **玩家在發牌過程中關閉遊戲會如何？** 該局繼續在後端完成，如果玩家已下注則正常結算，結果儲存於歷史紀錄，玩家下次登入時看到更新後的餘額
...
- **FR-006**: 系統必須使用包含8副撲克牌（共416張）的虛擬牌靴作為發牌來源
- **FR-007**: 系統必須按照標準百家樂規則依序發出四張牌（閒-莊-閒-莊）
- **FR-008**: 系統必須根據百家樂補牌規則自動判定是否需要為莊家或閒家補第三張牌
- **FR-009**: 系統必須正確計算牌面點數（A=1, 2-9=面值, 10/J/Q/K=0，總和取個位數）
- **FR-010**: 系統必須根據最終點數判定勝負（閒家勝/莊家勝/和局）

**補牌規則實作**

- **FR-011**: 當閒家前兩張牌點數為0-5時必須補牌，6-7不補牌，8-9為天生勝雙方不補牌
- **FR-012**: 當閒家未補牌且莊家點數0-5時必須補牌，6-7不補牌，8-9不補牌
- **FR-013**: 當閒家有補牌時，莊家必須根據自身點數和閒家第三張牌決定是否補牌：
  - 莊家0-2：一律補牌
  - 莊家3：閒家第三張非8時補牌
  - 莊家4：閒家第三張為2-7時補牌
  - 莊家5：閒家第三張為4-7時補牌
...
- **FR-014**: 閒家勝時，閒家投注區必須按1:1賠率發放獎金
- **FR-015**: 莊家勝時，莊家投注區必須按1:0.95賠率發放獎金（扣除5%抽水）
- **FR-016**: 和局時，和局投注區必須按1:8賠率發放獎金
- **FR-017**: 和局時，莊家和閒家投注區的下注金額必須退還給玩家（不計輸贏）
- **FR-018**: 系統必須在結算後更新玩家帳戶餘額並顯示本局輸贏金額

**使用者介面**

- **FR-019**: 系統必須顯示玩家當前帳戶餘額
- **FR-020**: 系統必須顯示每個下注區域的當前下注金額總和
- **FR-021**: 系統必須在發牌時以動畫方式逐張顯示牌面
- **FR-022**: 系統必須在遊戲結束時明確顯示莊家點數、閒家點數和勝負結果
- **FR-023**: 系統必須顯示最近10局的歷史紀錄，包含每局的勝負結果
- **FR-024**: 系統必須提供「開始新局」按鈕讓玩家進入下一輪遊戲

**牌靴與洗牌機制**

- **FR-025**: 系統必須在遊戲初始化時建立包含416張牌的虛擬牌靴並執行隨機洗牌
- **FR-026**: 系統必須在連續多局遊戲中使用同一個牌靴，依序從牌靴中發牌
- **FR-027**: 系統必須在牌靴剩餘牌數低於52張時自動觸發重新洗牌
- **FR-028**: 系統必須在每次洗牌時重置牌靴為416張牌並重新隨機排序

**錯誤處理**

- **FR-029**: 當發生牌組異常（如牌數不足無法完成發牌）時，系統必須宣告該局無效並退還所有下注金額
- **FR-030**: 當玩家在遊戲進行中關閉應用程式時，系統必須繼續完成該局並儲存結果，玩家下次登入時顯示更新後的餘額
...
- 玩家帳戶餘額由外部系統管理，本遊戲僅負責扣除和增加金額
- 遊戲為純客戶端實作，不需要與伺服器即時同步狀態
- 隨機數生成使用客戶端演算法（如Math.random()或PRNG），無需外部隨機數服務
- 遊戲不儲存玩家中途狀態，關閉應用程式後該局自動完成結算
