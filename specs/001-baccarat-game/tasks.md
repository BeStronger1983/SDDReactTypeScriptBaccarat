...
- [X] T013 [P] 配置 package.json scripts：dev, build, test, lint, format, type-check
...
- [X] T015 [P] 定義賠率常數 src/constants/payoutRates.ts（閒家 1:1、莊家 1:0.95、和局 1:8）
...
- [X] T027 **實作**: 牌靴管理 src/services/cardShoe.ts（createShoe, dealCard, needsShuffle）
...
- [X] T035 [P] **實作**: Chip 籌碼元件 src/components/ui/Chip.tsx（支援選中狀態）
...
- [X] T045 [US1] **實作**: 遊戲引擎 src/services/gameEngine.ts（executeGameRound, determineOutcome）

### 遊戲元件（TDD）

- [X] T046 [P] [US1] **測試**: BettingArea 元件測試 tests/unit/BettingArea.test.tsx
- [X] T047 [P] [US1] **實作**: BettingArea 下注區域元件 src/components/game/BettingArea.tsx（莊/閒/和局）
- [X] T048 [P] [US1] **測試**: ChipSelector 元件測試 tests/unit/ChipSelector.test.tsx
